...
// the primitives for exactly one of the available types are setup
  ...
  void ST7735S_clear(void) {
    ...
    ucg.setFont(ucg_font_7x13_mf);                        // _ one of 6,10,18,51,120,123,0 - font 0 is 5x16 linesxrows; font 10 is 7x21
  #else
...
void LCDprint(byte i) {
  ucg.print(i);
}
void LCDprintLine(const char *string) {
 ucg.print(string);
}
...
void LCDsetLine(byte line) { // Line = 1 or 2 - vt100 has lines 1-99
  if (line >= 1 && line <= (MULTILINE_PRE + MULTILINE_POST)) {
    ...
    ucg.setPrintPos(0, 13*line);
}
void LCDattributesBold() {
    ucg.setFont(ucg_font_7x13B_tr);
  }
...
void initLCD() {
  ...
  ST7735S_init();
#if defined(OLED_I2C_128x64) && !(defined(OLED_I2C_128x64LOGO_PERMANENT)) && defined(NEW_OLED_FONT) && defined(ST7735S) && !(defined(LCD_TELEMETRY))
