...
void evaluateCommand() {
  switch(cmdMSP[CURRENTPORT]) {
   ...
   case MSP_ATTITUDE:
     ...
     serialize16(heading);
     //serialize16(Compass_Heading); // [ 0..360 grd]
   ...
   case MSP_ALTITUDE:
     ...
     serialize32(baroPressure);          // in Pa
     serialize32(baroTemperature);       // in 0.01 degC
     //serialize32(Baro_P_mbara);           // in in mbar) 
     //serialize32(Baro_T_gradC);           // in degC
     break;
   case MSP_ANALOG:
     headSerialReply(5);
     serialize8(vbat);
   ...
   case MSP_ACC_CALIBRATION:
     // original -> if(!f.ARMED) calibratingA=512;
     calibratingA=512;
     headSerialReply(0);
     break;
   case MSP_MAG_CALIBRATION:
     // original -> if(!f.ARMED) f.CALIBRATE_MAG = 1;
     f.CALIBRATE_MAG = 1;
     headSerialReply(0);
     break;
#if defined(SPEK_BIND)
